...
class PriorityController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if (!empty($_POST['name']))
        {
            ...
            if($model->save()) {
                $i=1;
                foreach (Priority::find()->orderBy(['position'=>SORT_ASC])->all() as $recort)
                {
                    $recort->position = $i;
                    $recort->save();
                    $i++;
    ...
    public function actionUp($id)
    {
        ...
        if(!empty($active))
        {
            $model = Priority::find()->where('position < '.$active->position)->orderBy(['position'=>SORT_DESC])->one();
            if(!empty($model))
            {
    ...
    public function actionDown($id)
    {
        ...
        if(!empty($active))
        {
            $model = Priority::find()->where('position > '.$active->position)->orderBy(['position'=>SORT_ASC])->one();
            if(!empty($model))
            {
                $temp = $active->position;
                $active->position = $model->position;
