...
<section role="main" class="content-body">
  ...
  <section class="panel">
    <header class="panel-heading">
      ...
      <a class="btn btn-primary btn-xs" href="<?=\yii\helpers\Url::toRoute(['/access/home_page_item/create'])?>"><i class="fa fa-plus"></i>&nbsp;Добавить</a>
    </header>
    <div class="panel-body">
      <table class="table table-striped mb-none table-country" id="datatable-editable">
        ...
        {
          $url_name = \app\models\FilterItem::find()->where(['id'=>$record['url']])->asArray()->one();
          $category = \app\models\HomePageCategory::find()->where(['id'=>$record['category']])->asArray()->one();

          ?>
          <tr class="gradeX" data-numline="<?=$record['id']?>">
            <td class="hidden-2"><?=$record['id']?></td>
            <td class="text-vertical"><a href="/img/uploads/other/original/<?=unserialize($record['image'])[0]?>" class="image-popup-no-margins"><img width="30" alt="" src="/img/uploads/other/thumb/<?=unserialize($record['image'])[0]?>"></a></td>
            <td class="text-vertical"><?=$record['title']?></td>
            <td class="text-vertical"><?=$category['name']?></td>
            <td class="text-vertical">/catalog/proekty-domov/<?=$url_name['url']?></td>
            <td class="actions">
              <a href="<?=\yii\helpers\Url::toRoute(['/access/home_page_item/update','id'=>$record['id']])?>" class="on-default edit-row"><i class="fa fa-pencil"></i></a>
                ...
                change: function() {
                  _self.dialog.$confirm.on( 'click', function( e ) {
                    ...
                    $.ajax({
                      type: "POST",
                      dataType: 'json',
                      url: "/access/home_page_item/delete",
                      data:  {
                        id: $row.data("numline"),
