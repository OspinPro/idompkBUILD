    ...
    <section class="panel">
        ...
        <div class="panel-body">
            <table class="table table-striped mb-none table-city" id="datatable-editable">
                ...
                <tbody>
                <?php
                foreach(\app\models\DopInfo::find()->asArray()->all() as $country)
                {?>
                    <tr class="gradeA" data-numline="<?=$country['id']?>">
                        ...
                        <td class="actions">
                            ...
                            <a href="<?=\yii\helpers\Url::toRoute(['/access/dop_info/up','id'=>$country['id']])?>" class="on-default sortup-row"><i class="fa fa-arrow-up"></i></a>
                            <a href="<?=\yii\helpers\Url::toRoute(['/access/dop_info/down','id'=>$country['id']])?>" class="on-default sortdown-row"><i class="fa fa-arrow-down"></i></a>
...
<script>
                        ...
                        success: function(data,status) {
                            if(data.status!="error")
                            {
								...
								values[5] = [
									...
									'<a href="#" class="hidden on-editing cancel-row"><i class="fa fa-times"></i></a>',
									'<a href="#" class="on-default edit-row"><i class="fa fa-pencil"></i></a>',
									'<a href="/access/dop_info/up?id='+data.id+'" class="on-default sortup-row"><i class="fa fa-arrow-up"></i></a>',
									'<a href="/access/dop_info/down?id='+data.id+'" class="on-default sortdown-row"><i class="fa fa-arrow-down"></i></a>',
									'<a href="#" class="on-default remove-row"><i class="fa fa-trash-o"></i></a>'
								].join(' ');
    ...
    $(document).on("click touchstart", ".sort_link_hide", function(e){
        ...
        $.ajax({
            type: "POST",
            dataType: 'json',
            url: "/access/dop_info/reposition",
            data:  {
                mass: sortArray,
                mass2: sortArray2,
                _csrf: '<?=Yii::$app->request->getCsrfToken()?>'
