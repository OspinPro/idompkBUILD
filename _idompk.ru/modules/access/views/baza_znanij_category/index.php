    ...
    <section class="panel">
        ...
        <div class="panel-body">
            <table class="table table-striped mb-none table-city" id="datatable-editable">
                ...
                <tbody>
                <?php
                foreach(\app\models\BazaZnanijCategory::find()->asArray()->all() as $category)
                {?>
                    <tr class="gradeA" data-numline="<?=$category['id']?>">
                        <td class="hidden-2"><?=$category['position']?></td>
                        <td class="actions"><input class="form-control sort_inp pull-left pr-md input-sm" style="width:40%;" type="text" value="<?=$category['position']?>" /> <a class="btn btn-primary btn-sm sort_link_hide ml-md" href="#">Сохранить</a></td>
                        <td class="text-vertical"><?=$category['name']?></td>
                        <td class="actions">
                            <a href="#" class="hidden on-editing save-row"><i class="fa fa-save"></i></a>
                            <a href="#" class="hidden on-editing cancel-row"><i class="fa fa-times"></i></a>
                            <a href="#" class="on-default edit-row"><i class="fa fa-pencil"></i></a>
                            <a href="<?=\yii\helpers\Url::toRoute(['/access/baza_znanij_category/up','id'=>$category['id']])?>" class="on-default sortup-row"><i class="fa fa-arrow-up"></i></a>
                            <a href="<?=\yii\helpers\Url::toRoute(['/access/baza_znanij_category/down','id'=>$category['id']])?>" class="on-default sortdown-row"><i class="fa fa-arrow-down"></i></a>
...
<script>
                                ...
                                change: function() {
                                    _self.dialog.$confirm.on( 'click', function( e ) {
                                        ...
                                        $.ajax({
                                            type: "POST",
                                            dataType: 'json',
                                            url: "/access/baza_znanij_category/delete",
                                            data:  {
                                                id: $row.data("numline"),
			...
			/*	if (!values[1]) {
				...
				if (typeof attr !== typeof undefined && attr !== false) {
                    ...
                    $.ajax({
                        type: "POST",
                        dataType: 'json',
                        url: "/access/baza_znanij_category/update",
                        data:  {
                            name: values[2],
                            id: $row.data("numline"),
                            _csrf: '<?=Yii::$app->request->getCsrfToken()?>'
                    ...
                    $.ajax({
                        ...
                        async: false,
                        dataType: 'json',
                        url: "/access/baza_znanij_category/create",
                        data:  {
                            name: values[2],
                            _csrf: '<?=Yii::$app->request->getCsrfToken()?>'
                        },
                        success: function(data,status) {
                            if(data.status!="error")
                            {
								...
								values[5] = [
									...
									'<a href="#" class="hidden on-editing cancel-row"><i class="fa fa-times"></i></a>',
									'<a href="#" class="on-default edit-row"><i class="fa fa-pencil"></i></a>',
									'<a href="/access/baza_znanij_category/up?id='+data.id+'" class="on-default sortup-row"><i class="fa fa-arrow-up"></i></a>',
									'<a href="/access/baza_znanij_category/down?id='+data.id+'" class="on-default sortdown-row"><i class="fa fa-arrow-down"></i></a>',
									'<a href="#" class="on-default remove-row"><i class="fa fa-trash-o"></i></a>'
								].join(' ');
    ...
    $(document).on("click touchstart", ".sort_link_hide", function(e){
        ...
        $.ajax({
            type: "POST",
            dataType: 'json',
            url: "/access/baza_znanij_category/reposition",
            data:  {
                mass: sortArray,
                mass2: sortArray2,
                _csrf: '<?=Yii::$app->request->getCsrfToken()?>'
