    ...
    <section class="panel">
        ...
        <div class="panel-body">
            <table class="table table-striped mb-none table-city" id="datatable-editable">
                ...
                <tbody>
                <?php
                foreach(\app\models\BuildsSteps::find()->asArray()->all() as $country)
                {?>
                    <tr class="gradeA" data-numline="<?=$country['id']?>">
                        ...
                        <td class="actions">
                            ...
                            <a href="<?=\yii\helpers\Url::toRoute(['/access/builds_steps/up','id'=>$country['id']])?>" class="on-default sortup-row"><i class="fa fa-arrow-up"></i></a>
                            <a href="<?=\yii\helpers\Url::toRoute(['/access/builds_steps/down','id'=>$country['id']])?>" class="on-default sortdown-row"><i class="fa fa-arrow-down"></i></a>
...
<script>
                                ...
                                change: function() {
                                    _self.dialog.$confirm.on( 'click', function( e ) {
                                        ...
                                        $.ajax({
                                            type: "POST",
                                            dataType: 'json',
                                            url: "/access/builds_steps/delete",
                                            data:  {
                                                id: $row.data("numline"),
			...
			/*	if (!values[1]) {
				...
				if (typeof attr !== typeof undefined && attr !== false) {
                    ...
                    $.ajax({
                        type: "POST",
                        dataType: 'json',
                        url: "/access/builds_steps/update",
                        data:  {
                            name: values[2],
                            id: $row.data("numline"),
                            _csrf: '<?=Yii::$app->request->getCsrfToken()?>'
                    ...
                    $.ajax({
                        ...
                        async: false,
                        dataType: 'json',
                        url: "/access/builds_steps/create",
                        data:  {
                            name: values[2],
                            _csrf: '<?=Yii::$app->request->getCsrfToken()?>'
                        },
                        success: function(data,status) {
                            if(data.status!="error")
                            {
								...
								values[5] = [
									...
									'<a href="#" class="hidden on-editing cancel-row"><i class="fa fa-times"></i></a>',
									'<a href="#" class="on-default edit-row"><i class="fa fa-pencil"></i></a>',
									'<a href="/access/builds_steps/up?id='+data.id+'" class="on-default sortup-row"><i class="fa fa-arrow-up"></i></a>',
									'<a href="/access/builds_steps/down?id='+data.id+'" class="on-default sortdown-row"><i class="fa fa-arrow-down"></i></a>',
									'<a href="#" class="on-default remove-row"><i class="fa fa-trash-o"></i></a>'
								].join(' ');
    ...
    $(document).on("click touchstart", ".sort_link_hide", function(e){
        ...
        $.ajax({
            type: "POST",
            dataType: 'json',
            url: "/access/builds_steps/reposition",
            data:  {
                mass: sortArray,
                mass2: sortArray2,
                _csrf: '<?=Yii::$app->request->getCsrfToken()?>'
